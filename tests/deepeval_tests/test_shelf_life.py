...
from app import generate_recommendation, get_recipe_recommendations, parse_shelf_life, driver
...
def save_to_excel(row):
    ...
    output_file = './output/shelf_life_output.xlsx'
        ...
        with driver.session() as session:
            result = session.run("""
                ...
                WHERE toLower(trim(i.name)) CONTAINS toLower(trim(search_ing))
                   OR toLower(trim(search_ing)) CONTAINS toLower(trim(i.name))
...
def evaluate_urgent_usage(recipes, urgent_ingredients):
    """ measure if recipes use urgent ingredients"""
    if not urgent_ingredients or not recipes:
        return 0
    
    # Extract base urgent ingredient names 
    urgent_names = [ing.split(' (')[0].strip().lower() for ing in urgent_ingredients]
    
    total_urgent_used = 0
    for recipe in recipes[:3]:  # Check top 3 recipes
        recipe_ings = []
        ...
        # Count how many urgent ingredients are used in this recipe
        urgent_used = sum(1 for urgent in urgent_names if any(urgent in recipe_ing for recipe_ing in recipe_ings))
        total_urgent_used += urgent_used
    
    return total_urgent_used / len(urgent_names) if urgent_names else 0.0
...
def evaluate_scenario_with_shelf_life(scenario):
    ...
    try:
        ...
        recipes_used = [r['title'] for r in recipes[:3]] if recipes else []
        
        print(f"Recipes recommended: {recipes_used}")
        
        # Check what urgent ingredients each recipe has and why
        if recipes:
        ...
        urgent_ingredients_used = list(urgent_ingredients_used)
        ...
        # print(f"urgent ingredients from database: {all_urgent_ingredients}")
        
        # Calculate urgent usage rate
        urgent_usage_rate = evaluate_urgent_usage(recipes, all_urgent_ingredients)
        print(f"urgent usage: {urgent_usage_rate:.1%}")
        
    except Exception as e:
        print(f"Evaluation failed: {e}")

    # Create test case for LLM judge, add recipes in context
    ...
    try:
        ...
        save_to_excel(row)
        # print(f"results saved {scenario_id}")
    except Exception as e:
        print(f"Error saving to excel: {e}")
...
def load_test_cases():
    ...
    try:
        with open('./test_data/test_scenarios.json', 'r') as f:
            data = json.load(f)
            return data
    except Exception as e:
...
def run_shelf_life_evaluation():
    ...
    if all_results:
        shelf_life_scores = [r["llm_shelf_life_score"] for r in all_results]
        shelf_life_passed = [r for r in all_results if r["llm_shelf_life_score"] >= 1.0]
        
        # Calculate urgent ingredient usage statistics
        total_urgent_found = sum(len(r["urgent_ingredients_found"]) for r in all_results)
        total_urgent_used = sum(len(r["urgent_ingredients_used"]) for r in all_results)
        avg_urgent_usage = sum(r["urgent_usage_rate"] for r in all_results) / len(all_results)
    ...
    print(f"\n results saved to: ./output/shelf_life_evaluation.xlsx")
    return all_results
